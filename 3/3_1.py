...
def is_num_adjacent_to_any_sym(num, syms):
...
with open('input', 'r') as f:
    ...
    nums_adjacent_to_syms = []
    for i, num in enumerate(nums):
        if is_num_adjacent_to_any_sym(num, syms):
            nums_adjacent_to_syms.append(num)

    plot(nums_adjacent_to_syms, syms, lines)
    
    print(sum([num[0] for num in nums_adjacent_to_syms]))
